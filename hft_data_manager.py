...
class HFTDataManager:
    ...
    def __init__(self):
        self.data_buffers: Dict[str, Deque] = {
            ...
            '60s': deque(maxlen=400),
            '300s': deque(maxlen=240),
            '900s': deque(maxlen=200),
    ...
    def update_orderbook(self, orderbook: Dict):
        ...
        self.orderbook_state = {
            ...
            'bid_volume_top3': orderbook.get('bid_volume_top3'),
            'ask_volume_top3': orderbook.get('ask_volume_top3')
        }

    def get_orderbook_metrics(self) -> Dict:
        return dict(self.orderbook_state)

    def get_timeframe_bars(self, key: str) -> Deque:
        return self.data_buffers.get(key, deque())
    ...
    async def _aggregate_higher_timeframes(self):
        ...
        self._aggregate_bars('1s', '60s', 60)
        self._aggregate_bars('60s', '300s', 5)
        self._aggregate_bars('300s', '900s', 3)
    ...
    def calculate_atr(self, timeframe: str = '60s', window: int = 14) -> Optional[float]:
        bars = self.data_buffers.get(timeframe)
    ...
    def estimate_tick_volatility(self, window: int = 30) -> float:
        ticks = self.data_buffers.get('ticks')
        if not ticks or len(ticks) < 2:
            return 0.0
        recent = list(ticks)[-max(window, 2):]
        changes: List[float] = []
        for idx in range(1, len(recent)):
            prev = recent[idx - 1].get('price') or 0.0
            curr = recent[idx].get('price') or 0.0
            if prev > 0:
